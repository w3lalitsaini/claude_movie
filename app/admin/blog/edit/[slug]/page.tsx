...
export default function EditBlogPage() {
  ...
  if (loading) {
    return (
      <div className="pt-28 p-6 flex justify-center">
    ...
    <div className="pt-28 p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <button onClick={() => router.back()} className="w-10 h-10 bg-[#1a1a1a] border border-[#222] text-[#888] rounded-sm flex items-center justify-center hover:text-white transition-colors">
            <FiArrowLeft size={18} />
          </button>
          <div>
            <h1 className="font-display font-bold text-2xl text-white uppercase tracking-widest">Edit Blog Post</h1>
            <p className="text-[#555] text-sm">Update existing post content</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button type="button" onClick={(e) => handleSubmit(e)} disabled={saving} className="btn-red flex items-center gap-2 text-white font-bold px-6 py-2.5 rounded-sm text-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div>
            <label className="text-[#666] text-xs uppercase tracking-wider font-semibold mb-1.5 block">Status</label>
        ...
        <div>
          <label className="text-[#666] text-xs uppercase tracking-wider font-semibold mb-1.5 block">Featured Image</label>
          <div className="flex gap-3">
            {form.featuredImage && (
              <img src={form.featuredImage} alt="Preview" className="w-12 h-12 object-cover rounded-sm border border-[#2a2a2a]" />
            )}
            <input value={form.featuredImage} onChange={e => setForm(p => ({...p, featuredImage: e.target.value}))} placeholder="Image URL"
        ...
        <div>
          <label className="text-[#666] text-xs uppercase tracking-wider font-semibold mb-1.5 block">Content (Markdown)</label>
          <textarea value={form.content} onChange={e => setForm(p => ({...p, content: e.target.value}))} required rows={15}
        ...
        <div className="border-t border-[#1a1a1a] pt-6">
          <p className="text-[#666] text-xs uppercase tracking-wider font-semibold mb-3">SEO Meta</p>
          <div className="space-y-3">
            <input value={form.metaTitle} onChange={e => setForm(p => ({...p, metaTitle: e.target.value}))} placeholder="Meta Title"
              className="w-full bg-[#0a0a0a] border border-[#2a2a2a] focus:border-[#e50914] text-white text-sm px-4 py-3 rounded-sm outline-none transition-colors" />
            <textarea value={form.metaDescription} onChange={e => setForm(p => ({...p, metaDescription: e.target.value}))} rows={2} placeholder="Meta Description"
              className="w-full bg-[#0a0a0a] border border-[#2a2a2a] focus:border-[#e50914] text-white text-sm px-4 py-3 rounded-sm outline-none resize-none transition-colors" />
          </div>
        </div>
      </form>
